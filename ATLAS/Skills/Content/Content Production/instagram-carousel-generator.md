...
**Setup (run once):**
```bash
npm install satori @fontsource/plus-jakarta-sans @fontsource/playfair-display @resvg/resvg-js
...
### Fonts — JANI Brand (PFLICHT, nie Outfit oder andere Fonts)

**Primär: Plus Jakarta Sans** (`@fontsource/plus-jakarta-sans`)
- Weights: 400, 500, 600, 700, 800 — ALWAYS load BOTH `latin` AND `latin-ext`
- Headlines: weight 800, letterSpacing `-0.03em`
- Sublines/Body: weight 500-600
- Labels/Badges: weight 700, letterSpacing `0.12em`, UPPERCASE

**Sekundär: Playfair Display** (`@fontsource/playfair-display`) — NUR italic
- Weights: 400, 500, 700 (italic only)
- Einsatz: Zahlen-Highlights, dekorative Akzente, Slide-Nummern
- NIEMALS regular — immer italic

```javascript
// Font-Setup (copy-paste für jedes generate.js)
const pjsDir = path.join(nmDir, 'node_modules/@fontsource/plus-jakarta-sans/files');
const pdDir  = path.join(nmDir, 'node_modules/@fontsource/playfair-display/files');
const fonts = [
  ...[400,500,600,700,800].flatMap(w => [
    { name:'Plus Jakarta Sans', weight:w, style:'normal',
      data: fs.readFileSync(path.join(pjsDir, `plus-jakarta-sans-latin-${w}-normal.woff`)) },
    { name:'Plus Jakarta Sans', weight:w, style:'italic',
      data: fs.readFileSync(path.join(pjsDir, `plus-jakarta-sans-latin-${w}-italic.woff`)) },
  ]),
  ...[400,500,700].map(w => ({
    name:'Playfair Display', weight:w, style:'italic',
    data: fs.readFileSync(path.join(pdDir, `playfair-display-latin-${w}-italic.woff`)),
  })),
];
// nmDir = '/sessions/funny-epic-meitner' (wo npm install lief)
```

### Color Palette — JANI Brand (aus [C] Jani Jäckel Brand Style Guide.md)

KEINE generischen Schwarz/Weiß-Farben — immer die Brand-Tokens verwenden.
Keine bunten Akzentfarben außer für funktionale Elemente.

| Token | Hex | Verwendung |
|-------|-----|-----------|
| navy | #0f1f35 | Dark-Slides Hintergrund, Headlines, Buttons |
| navy2 | #162942 | Sekundäre dunkle Flächen |
| navy3 | #0a1728 | Dunkelster Ton für Tiefe |
| navyLight | #eef2f7 | Helle Badges, Hintergründe auf Light-Slides |
| bg | #ffffff | Slide-Hintergrund (light) |
| bg2 | #f7f5f2 | Warm Off-White — Standard für helle Slides |
| border | #e8e4dd | Card-Borders, Trennlinien |
| text | #0f1f35 | Haupttext (identisch mit navy) |
| muted | #7a7264 | Labels, Captions, dezente Elemente |
| body | #595347 | Fließtext |

```javascript
// Brand Color Tokens (copy-paste)
const C = {
  navy:'#0f1f35', navy2:'#162942', navy3:'#0a1728', navyLight:'#eef2f7',
  bg:'#ffffff', bg2:'#f7f5f2', border:'#e8e4dd',
  text:'#0f1f35', muted:'#7a7264', body:'#595347',
};
```

### Dark Slides — Dot-Grid PFLICHT

Jede dunkle Slide (navy background) MUSS das Dot-Grid haben:
```javascript
const dotSvg = `<svg width="1080" height="1350" xmlns="http://www.w3.org/2000/svg">
  ${Array.from({length:49}, (_,row) => Array.from({length:39}, (_,col) =>
    `<circle cx="${col*28}" cy="${row*28}" r="1" fill="rgba(255,255,255,0.055)"/>`
  ).join('')).join('')}
</svg>`;
const dotSrc = `data:image/svg+xml;base64,${Buffer.from(dotSvg).toString('base64')}`;
// Einbetten als absolut positioniertes img hinter dem Content-Layer
```
...
7. **Hook-Research für Slide-1-Headline (PFLICHT)** — WebSearch nach Outlier-Videos/Posts zum Thema, Mechanismus extrahieren, Jani-Adaptation vorschlagen. Prozess: `Skills Library/Content/Content Ideation & Strategy/content-hook-research.md`. Niemals Slide-1-Headline aus dem Kopf generieren.
8. **Only then: write the code**

---

## COPY-REGELN — PFLICHT FÜR JEDE SLIDE

### Kein Staccato
Fragment-Sätze sind verboten. Nie: "Mehr denken. Weniger tippen. Besser klingen."
Immer: vollständige Sätze die einen Gedanken zu Ende führen.

❌ "Kompetent. Informiert. Ich hab das doch gelesen."
✅ "Du fühlst dich kompetent, weil du es gerade gelesen hast."

❌ "Kein Lernen. Kein Verständnis. Von Null."
✅ "Beim nächsten Mal stehst du genauso da wie vorher, weil kein echtes Verständnis entstanden ist."

### Bildlich schreiben
Abstrakte Aussagen brauchen immer ein konkretes Bild. Vor jeder Copy-Zeile fragen:
"Kann sich jemand das vorstellen, der das Thema nicht kennt?"

❌ "Verstehen reicht nicht."
✅ "Es nur zu verstehen bedeutet nicht, dass du dich in der Klausur in 3 Wochen dran erinnerst."

### Key Learnings = vollständige Sätze
Der Key-Learning-Bar am unteren Rand ist KEIN Slogan. Er ist eine Aussage.

❌ "Denken. Dann AI."
✅ "Wer denkt, bevor er fragt, bekommt von AI das, was er wirklich braucht."
...
async function main() {
  ...
  // JANI Brand Fonts — Plus Jakarta Sans + Playfair Display italic
  const nmDir  = '/sessions/funny-epic-meitner'; // wo npm install lief
  const pjsDir = path.join(nmDir, 'node_modules/@fontsource/plus-jakarta-sans/files');
  const pdDir  = path.join(nmDir, 'node_modules/@fontsource/playfair-display/files');
  const fonts = [
    ...[400,500,600,700,800].flatMap(w => [
  ...
  // JANI Brand Colors (aus [C] Jani Jäckel Brand Style Guide.md)
  const C = {
    navy:'#0f1f35', navy2:'#162942', navy3:'#0a1728', navyLight:'#eef2f7',
    bg:'#ffffff', bg2:'#f7f5f2', border:'#e8e4dd',
    text:'#0f1f35', muted:'#7a7264', body:'#595347',
