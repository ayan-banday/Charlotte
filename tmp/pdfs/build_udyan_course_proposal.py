...
ROOT = r'S:\Charlotte'
OUT = os.path.join(ROOT, 'output', 'pdf', 'Udyaan_From_Idea_to_Business_Proposal.pdf')
ART = os.path.join(ROOT, 'assets', 'udyan-course', 'ash-journey-cover.png')
BG_ART = os.path.join(ROOT, 'assets', 'udyan-course', 'night-expedition-background.png')
ASH_ART = os.path.join(ROOT, 'assets', 'udyan-course', 'ash-character.png')
W, H = A4
PAGE_BG = colors.HexColor('#171C26'); SURFACE = colors.HexColor('#252D3A'); PANEL = colors.HexColor('#303A49')
NAVY = colors.HexColor('#F0E8D8'); CREAM = PAGE_BG; INK = colors.HexColor('#F0E8D8')
MUTED = colors.HexColor('#B7B8BD'); GREEN = colors.HexColor('#75B8AE'); ORANGE = colors.HexColor('#D9A55D')
CORAL = colors.HexColor('#B96C68'); PURPLE = colors.HexColor('#9182B6'); PALE_GREEN = colors.HexColor('#263F43')
PALE_ORANGE = colors.HexColor('#443A2C'); PALE_CORAL = colors.HexColor('#453137'); PALE_PURPLE = colors.HexColor('#373344')
...
def footer(c, page):
    ...
    font(c, 7.5, MUTED); c.drawString(42, 18, 'UDYAAN | FROM IDEA TO BUSINESS'); c.drawRightString(W-42, 18, str(page))
...
def table(c, x, y, widths, rows, row_h=40, header=True):
    ...
    for ri, row in enumerate(rows):
        fill = colors.HexColor('#0E141E') if ri==0 and header else (SURFACE if ri%2 else PANEL)
        ...
        for ci, cell in enumerate(row):
            ...
            para(c, cell, xx+7, yy-15, widths[ci]-14, 7.6 if ri else 7.5, 10, PAPER if ri==0 and header else INK, ri==0 and header)
            xx += widths[ci]
        yy -= row_h
    return yy
...
def paint_bg(c):
    if os.path.exists(BG_ART):
        c.drawImage(ImageReader(BG_ART), 0, 0, width=W, height=H, preserveAspectRatio=False, mask='auto')
    else:
        c.setFillColor(PAGE_BG); c.rect(0,0,W,H,fill=1,stroke=0)

def flow(c, labels, x, y, width, accent):
    gap = 8; bw = (width-gap*(len(labels)-1))/len(labels)
    for i, label in enumerate(labels):
        xx=x+i*(bw+gap); c.setFillColor(SURFACE); c.setStrokeColor(accent); c.setLineWidth(1.2); c.roundRect(xx,y-30,bw,30,7,fill=1,stroke=1)
        font(c,7.5,PAPER,True); c.drawCentredString(xx+bw/2,y-19,label)
        if i < len(labels)-1:
            c.setStrokeColor(accent); c.setLineWidth(1.2); c.line(xx+bw+2,y-15,xx+bw+gap-3,y-15)
            c.line(xx+bw+gap-7,y-12,xx+bw+gap-3,y-15); c.line(xx+bw+gap-7,y-18,xx+bw+gap-3,y-15)

def mind_node(c, x, y, label, body, accent, radius=34):
    c.setFillColor(SURFACE); c.setStrokeColor(accent); c.setLineWidth(1.5); c.circle(x,y,radius,fill=1,stroke=1)
    font(c, 10, PAPER, True); c.drawCentredString(x, y+3, label)

c=canvas.Canvas(OUT, pagesize=A4); c.setTitle('Udyaan - From Idea to Business Proposal')
page=1
# 1 cover
c.setFillColor(CREAM); c.rect(0,0,W,H,fill=1,stroke=0)
font(c, 28, NAVY, True); c.drawString(50, H-70, 'FROM IDEA TO BUSINESS')
para(c, 'A practical entrepreneurship course for Udyaan students', 50, H-95, 240, 12, 16, INK)
...
# 2 transformation
paint_bg(c); title(c,'The transformation','Students arrive excited about entrepreneurship, but many have incomplete mental models of what a business is. The course turns that uncertainty into a sequence of real decisions.')
...
# 3 journey overview
paint_bg(c); title(c,'The journey','The course is presented as a story. Ash is the student avatar: motivated, intelligent, and initially eager to build too soon. Each story beat corresponds to a real business capability and a visible artifact.')
if os.path.exists(ASH_ART): c.drawImage(ImageReader(ASH_ART), 50, 250, width=185, height=300, preserveAspectRatio=True, mask='auto')
font(c, 16, NAVY, True); c.drawString(72, 235, 'THIS IS ASH')
para(c, 'Ash is the student avatar: curious, under-equipped, and willing to learn by acting. Ash uses a compass, a notebook, and real evidence to move through the business world.', 68, 215, 180, 9.2, 13, MUTED)
font(c, 14, NAVY, True); c.drawString(300, 660, 'ASH\'S BUSINESS MAP')
c.setStrokeColor(colors.HexColor('#B96C68')); c.setLineWidth(2)
for x1,y1,x2,y2 in [(380,510,445,590),(380,510,485,490),(380,510,445,390)]: c.line(x1,y1,x2,y2)
c.setFillColor(PALE_ORANGE); c.setStrokeColor(ORANGE); c.circle(380,510,47,fill=1,stroke=1)
font(c, 15, NAVY, True); c.drawCentredString(380,515,'ASH')
mind_node(c,445,590,'MAP','Choose territory. Find the problem.',GREEN,38)
mind_node(c,485,490,'FORGE','Create the offer.',ORANGE,38)
mind_node(c,445,390,'BATTLE','Sell, build, learn.',CORAL,38)
para(c, 'Choose territory. Find the problem.', 400, 540, 145, 8.5, 11, MUTED)
para(c, 'Create the offer.', 455, 430, 120, 8.5, 11, MUTED)
para(c, 'Sell, build, learn.', 405, 330, 130, 8.5, 11, MUTED)
para(c, 'The story is not a straight line. Ash returns to the map after every test, carrying better evidence and a sharper next move.', 285, 300, 245, 10, 14, INK, False, True)
font(c, 10, MUTED, True); c.drawString(285, 245, 'THREE PHASES. ONE REPEATABLE CAPABILITY.')
newpage(c,page); page+=1
# phase 1
paint_bg(c); phase_banner(c,'1','MAP THE TERRITORY','Ash begins with a product idea. The market is covered in fog. Before building, Ash must discover who is in the territory and which problem is worth solving.',GREEN)
...
newpage(c,page); page+=1
# phase 2
paint_bg(c); phase_banner(c,'2','FORGE THE WEAPON','Ash has found a real problem. Now the weapon must be designed for this particular fight. The weapon is not merely a product - it is the complete offer.',ORANGE)
font(c, 14, NAVY, True); c.drawString(50, H-180, 'PROBLEM BEING SOLVED')
...
newpage(c,page); page+=1
# phase 3
paint_bg(c); phase_banner(c,'3','ENTER THE BATTLE','The weapon has never been tested. Ash must approach real customers, ask for commitment, deliver a first version, and learn what needs to change.',CORAL)
font(c, 14, NAVY, True); c.drawString(50, H-180, 'PROBLEM BEING SOLVED')
...
newpage(c,page); page+=1
# teaching model
paint_bg(c); title(c,'How the course is taught','The course is hybrid: written lessons establish concepts, short Loom videos make the thinking visible, and attached GPTs help students practise and improve real work.')
...
newpage(c,page); page+=1
# final outputs
paint_bg(c); title(c,'What every student must leave with','The course does not promise that every idea will succeed. It promises that every student will experience the business cycle and leave with stronger evidence about what to do next.')
...
footer(c,page); c.showPage(); page+=1
# timing page
paint_bg(c); title(c,'Timing and build estimate','A working estimate for planning the student journey and explaining the build effort. The first cohort will refine these numbers through real delivery.')
card(c,50,H-150,240,116,'STUDENT WORKLOAD','13 subphases x 3 hours each = approximately 39 active hours. This includes lesson work, GPT practice, and practical assignments.',PALE_GREEN,GREEN)
card(c,310,H-150,240,116,'AT 90 MINUTES / DAY','39 hours / 1.5 hours per day = 26 study days. At five study days per week, that is approximately 5.2 active weeks.',PALE_ORANGE,ORANGE)
font(c, 14, NAVY, True); c.drawCentredString(W/2, H-305, 'ACTIVE LEARNING PATH')
flow(c, ['PHASE 1 - 12 HRS','PHASE 2 - 12 HRS','PHASE 3 - 15 HRS'], 58, H-335, W-116, CORAL)
para(c, 'Map the territory', 75, H-390, 130, 9, 12, MUTED, True)
para(c, 'Forge the offer', 245, H-390, 130, 9, 12, MUTED, True)
para(c, 'Sell, build, learn', 420, H-390, 130, 9, 12, MUTED, True)
card(c,50,310,500,72,'REAL-WORLD CALENDAR','Customer scheduling, waiting for replies, feedback, and iteration extend the calendar. Plan on approximately 6-7 weeks for the first cohort.',PALE_CORAL,CORAL)
font(c, 14, NAVY, True); c.drawCentredString(W/2, 215, 'COURSE PRODUCTION ESTIMATE')
para(c, 'Creating the lessons, Looms, GPT prompts, artifact templates, rubrics, and QA is provisionally 50-65 creator hours: 39 hours of core design plus 10-25 hours of integration and testing.', 65, 185, W-130, 10, 14, INK)
